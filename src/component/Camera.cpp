...
#include "../../include/vox-engine/component/Camera.h"
...
void Camera::init() {
    ...
    _transform = myEntity->transform;

    Display::addDisplayAction([](unsigned int width, unsigned int height) {
        if (width != 0 && height != 0){
...
Matrix4f Camera::getViewMatrix() {
    if (_transform->hasChangedLastTick()) {
        _viewMatrix = Matrix4f::lookAt(_transform->getPositionWorld(),
                                       _transform->up(), _transform->forward());
    }
...
Vector4f Camera::getProjectionExtents(float xOffset, float yOffset) {
    ...
    return Vector4f(oneExtentX, oneExtentY, oneJitterX,
...
Vector2f Camera::worldToScreen(Vector3f point) {
    ...
    return {clipSpacePos.x() / clipSpacePos.w(), clipSpacePos.y() / clipSpacePos.w()};
}

void Camera::updateFrustum() {
    if (_transform->hasChangedSince(_lastFrustumUpdate)) {
...
bool Camera::frustumTest(ObjectRenderer* object) {
    ...
    Transform* transform = object->getEntity()->transform;

    float maxScaleAxis = Math::max(
            Math::max(transform->getScale().x(),
            transform->getScale().y()),
            transform->getScale().z());

    Vector3f center = (transform->getTransformationMatrix() *
                       Vector4f(object->getBoundingSphere().getCenter(), 1.0f)).xyz();

    for (size_t i = 0; i < 5; ++i) {
        if (_frustum[i].normal.dot(center) + _frustum[i].distance +
            maxScaleAxis * object->getBoundingSphere().getRadius() < 0) {
            return false;
        }
    }
